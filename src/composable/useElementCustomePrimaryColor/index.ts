...
const mix = (color1: string, color2: string, weight: number) => {
  ...
  let r = Math.round(r1 * (1 - weight) + r2 * weight) as string | number
  let g = Math.round(g1 * (1 - weight) + g2 * weight) as string | number
  let b = Math.round(b1 * (1 - weight) + b2 * weight) as string | number
